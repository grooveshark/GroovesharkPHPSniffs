 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class Grooveshark_Sniffs_Functions_FunctionDeclarationArgumentSpacingSniff implements PHP_CodeSniffer_Sniff
{
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        while (($nextParam = $phpcsFile->findNext(T_VARIABLE, ($nextParam + 1), $closeBracket)) !== false) {
            ...
            if ($nextCode === T_EQUAL) {
                ...
                if (($nextToken - $nextParam) > 2 || ($nextToken - $nextParam) < 2) {
                    $error = 'Expected 1 spaces between argument "%s" and equals sign; %s found';
                    if (($nextToken - $nextParam) > 1) {
                        $strlen = strlen($tokens[($nextParam + 1)]['content']);
                    } else {
                        $strlen = 0;
                    }
                    $data  = array(
                ...
                if ($tokens[($nextToken + 1)]['content'] !== ' ') {
                    $error = 'Expected 1 spaces between default value and equals sign for argument "%s"; %s found';
                    if ($tokens[($nextToken + 1)]['code'] == T_WHITESPACE) {
                        $len = strlen($tokens[($nextToken + 1)]['content']);
                    } else {
                        $len = 0;
                    }
                    $data  = array(
