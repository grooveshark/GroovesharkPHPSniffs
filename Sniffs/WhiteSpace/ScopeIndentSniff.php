 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class Grooveshark_Sniffs_WhiteSpace_ScopeIndentSniff implements PHP_CodeSniffer_Sniff
{
        ...
        for ($i = ($scopeOpener + 1); $i < $scopeCloser; $i++) {
            ...
            if ($newline === true && $tokens[$i]['code'] !== T_WHITESPACE) {
                ...
                if ($column !== $indent) {
                    ...
                    if ($tokens[$firstToken]['code'] === T_CONSTANT_ENCAPSED_STRING) {
                        $acceptable = true;
                    } elseif (in_array($tokens[$firstToken]['code'], $unknowns) && in_array($tokens[$firstToken + 1]['code'], $unknowns)) {
                        $acceptable = true;
                    }
                    if (!$acceptable) {
                        if ($this->exact === true || $column < $indent || $column > $indent) {
                            $type  = 'IncorrectExact';
